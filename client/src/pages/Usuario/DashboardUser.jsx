...
const DashboardUser = () => {
	...
	const categorias = [
		"todas",
		...new Set(servicos.map((servico) => servico.category_id)),
	];
	...
	const handleSolicitarServico = (servico) => {
		...
		const novaOS = {
			...
			address: endereco,
			notes: mensagem || "Solicitação automática",
		};
	...
	const renderPaginacao = () => {
		...
		return (
			<div className={styles.paginacaoContainer}>
				...
				{Array.from({length: fim - inicio + 1}, (_, i) => inicio + i).map(
					(num) => (
						<button
							key={num}
							onClick={() => setPaginaAtual(num)}
							className={`${styles.botaoPagina} ${
								paginaAtual === num ? styles.paginaAtiva : ""
							}`}
						>
							{num}
						</button>
			...
			{aba === "servicos" && (
				<>
							...
							servicosPagina.map((servico) => (
								<div key={servico.id} className={styles["servico-card"]}>
									...
									<div className={styles["servico-info"]}>
										<h3>{servico.title}</h3>
										<p className={styles["servico-descricao"]}>
											{servico.description}
										</p>
										<div className={styles["servico-metadados"]}>
											<span>
												<strong>Valor:</strong> R$ {servico.price_min} - R${" "}
												{servico.price_max}
											</span>
											<span>
												<strong>Categoria:</strong> {servico.category_id}
											</span>
										</div>
									</div>
					...
					<div className={styles.infoPaginacao}>
						Mostrando {servicosPagina.length} de {servicosFiltrados.length}{" "}
						serviços
						{filtroCategoria !== "todas" &&
								...
								<div key={ordem.id} className={styles["ordem-card"]}>
									<div className={styles["ordem-info"]}>
										...
										<div className={styles["ordem-metadados"]}>
											...
											<span>
												Data:{" "}
												{new Date(ordem.scheduled_date).toLocaleDateString()}
											</span>
										</div>
									</div>
									<div className={styles["ordem-actions"]}>
										...
										{ordem.status === "completed" && ordem.avaliada && (
											<span className={styles.avaliacaoEnviada}>
												✅ Avaliação enviada
											</span>
										)}
									</div>
					...
					<div className={styles["modal"]}>
						...
						<div className={styles["modal-content"]}>
							<div className={styles["modal-imagem-container"]}>
								<img
									src={
										servicoSelecionado.images &&
										typeof servicoSelecionado.images === "string"
							...
							<div className={styles["modal-info"]}>
								...
								<p className={styles["modal-descricao"]}>
									{servicoSelecionado.description}
								</p>
								<div className={styles["modal-detalhes"]}>
									<div>
										<strong>Valor:</strong> R$ {servicoSelecionado.price_min} -
										R$ {servicoSelecionado.price_max}
									</div>
									<div>
									...
									<div className={styles["avaliacao-estrelas"]}>
										{[1, 2, 3, 4, 5].map((n) => (
											<span
												...
												onClick={() => setNota(n)}
												className={
													n <= nota ? styles.estrelaAtiva : styles.estrela
												}
											>
												{n <= nota ? "★" : "☆"}
											</span>
