...
const PerfilPrestador = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.provider) {
          setForm({
            bio: res.data.provider.bio || "",
            cnpj: res.data.provider.cnpj || "",
            experiencia: res.data.provider.experience || "",
            categorias: res.data.provider.categories
              ? res.data.provider.categories.map((c) => c.id)
              : [],
          });
        }
      });
  ...
  // Busca categorias
  useEffect(() => {
    axios.get("http://localhost:3333/categories").then((res) => setCategorias(res.data));
  }, []);
  ...
  const validarPerfil = () => {
    ...
    if (!form.experiencia)
      erros.experiencia = "Informe sua experiência.";
  ...
  if (!dados) return <div className={styles.dashboardPrestador}>Carregando...</div>;
      ...
      <header className={styles.header}>
        ...
        <nav className={styles.navegacao}>
          <button
            ...
            className={`${styles.botaoNavegacao} ${aba === "perfil" ? styles.ativo : ""}`}
          >
            Perfil do Prestador
          </button>
          <button
            onClick={() => setAba("ordens")}
            className={`${styles.botaoNavegacao} ${aba === "ordens" ? styles.ativo : ""}`}
          >
            Histórico de Ordens
          </button>
          <button
            onClick={() => setAba("avaliacoes")}
            className={`${styles.botaoNavegacao} ${aba === "avaliacoes" ? styles.ativo : ""}`}
          >
            ...
            {!editando ? (
              <div className={styles.perfilDados}>
                ...
                <div className={styles.dadoItem}>
                  ...
                  <p className={styles.dadoValor}>{dados.provider?.status || "-"}</p>
                </div>
                <div className={styles.dadoItem}>
                ...
                <div className={styles.dadoItem}>
                  ...
                  <p className={styles.dadoValor}>{form.experiencia || "-"}</p>
                </div>
                <div className={styles.dadoItem}>
                  <span className={styles.dadoLabel}>Categorias:</span>
                  <p className={styles.dadoValor}>
                    {categorias.filter(cat => form.categorias.includes(cat.id)).map(cat => cat.name).join(", ") || "-"}
                  </p>
                </div>
                <button className={styles.botaoPrimario} onClick={() => setEditando(true)}>
                  {form.bio && form.categorias.length > 0 ? "Editar Perfil" : "Completar Perfil"}
                </button>
              </div>
            ) : (
              <form className={styles.formulario} onSubmit={handleSalvar}>
                <div className={styles.grupoFormulario}>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className={`${styles.entradaTexto} ${errosPerfil.bio ? styles.inputErro : ""}`}
                  />
                  {errosPerfil.bio && (
                    <span className={styles.mensagemErro}>{errosPerfil.bio}</span>
                  )}
                </div>
                <div className={styles.grupoFormulario}>
                  ...
                  {errosPerfil.categorias && (
                    <span className={styles.mensagemErro}>{errosPerfil.categorias}</span>
                  )}
                </div>
                <div className={styles.grupoFormulario}>
                  <label htmlFor="experiencia" className={styles.rotulo}>
                    Anos de experiência <span style={{ color: "red" }}>*</span>
                  </label>
                  <select
                    id="experiencia"
                    name="experiencia"
                    ...
                    onChange={handleChange}
                    className={`${styles.entradaTexto} ${errosPerfil.experiencia ? styles.inputErro : ""}`}
                  >
                  ...
                  {errosPerfil.experiencia && (
                    <span className={styles.mensagemErro}>{errosPerfil.experiencia}</span>
                  )}
                </div>
                <div className={styles.grupoFormulario}>
                  ...
                  />
                  {errosPerfil.cnpj && (
                    <span className={styles.mensagemErro}>{errosPerfil.cnpj}</span>
                  )}
                </div>
