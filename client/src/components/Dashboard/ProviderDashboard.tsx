...
export function ProviderDashboard({ setCurrentView }: { setCurrentView: (view: string) => void }) {
  ...
  useEffect(() => {
    async function fetchTotals() {
      try {
        ...
        const resServices = await fetch("http://localhost:3333/services/total", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
  ...
  useEffect(() => {
    async function fetchPendingOrders() {
      try {
        ...
        console.log("🔍 pendingOrders data:", data);
  ...
  const handleUpdateService = async (updatedService: any) => {
    try {
      ...
      setMyServices((prev) => prev.map((s) => (s.id === updated.id ? updated : s)));
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {providerStatus === "pending" && <ProfileAlert onCompleteProfile={() => setCurrentView("profile")} />}

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatsCard icon={Calendar} iconColor="text-blue-600" title="Ordens Pendentes" value={pendingCount.toString()} />
        <StatsCard icon={Users} iconColor="text-green-600" title="Total de Ordens Recebidas" value={totalOrders.toString()} />
        <StatsCard icon={DollarSign} iconColor="text-orange-600" title="Total de Serviços" value={totalServices.toString()} />
        <StatsCard icon={Star} iconColor="text-yellow-600" title="Avaliação Média" value={averageRating} />
      </div>
...
<Card>
  ...
  <CardContent>
    <div className="space-y-4">
      ...
      <ServiceEditModal open={showEdit} service={selectedService} onClose={() => setShowEdit(false)} onSave={handleUpdateService} />
    </div>
  );
}
