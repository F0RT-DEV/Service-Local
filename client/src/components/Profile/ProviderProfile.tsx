...
export function ProviderProfile() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data && data.user && data.provider) {
          ...
          setProviderStatus(data.provider.status || "pending"); // seta status do provider
        }
      } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!resProvider.ok) throw new Error("Erro ao atualizar dados do prestador");
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-sm text-green-700">
                Seu perfil foi aprovado! Agora você pode receber ordens normalmente.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className="w-full border rounded px-3 py-2" value={profileData.address} disabled />
              </div>
              <div>
                <label className="block text-sm font-medium">CPF</label>
                <input className="w-full border rounded px-3 py-2" value={cpf} disabled />
              </div>
            </div>
          </div>
